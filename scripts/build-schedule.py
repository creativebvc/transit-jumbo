...
def station_label(stop_name):
    """Turn 'NB Saddletowne CTrain Station' into 'Saddletowne'."""
    n = (stop_name or "").strip()
    n = re.sub(r"\(Free Fare Zone\)", "", n)
    n = re.sub(r"^(WB|EB|NB|SB)\s+", "", n, flags=re.I)
    n = re.sub(r"\s*(CTrain\s*)?Sta(tion|ion|_)?\s*(NB|SB|EB|WB)?\s*$", "", n, flags=re.I)
    return n.strip(" -") or stop_name
...
def main(gtfs_dir):
    ...
    # ---- stop names (used to repair mislabelled headsigns) ------------------
    stop_names = {}
    if os.path.exists(path("stops.txt")):
        with open(path("stops.txt"), newline="", encoding="utf-8-sig") as f:
            for r in csv.DictReader(f):
                stop_names[r["stop_id"]] = r.get("stop_name", "")
    ...
    # Single pass over stop_times: collect candidate rows at our two platforms,
    # and simultaneously track each trip's final stop_sequence.
    #
    # Trips that TERMINATE at City Hall are then dropped. Calgary runs short-turn
    # service signed "City Hall" that ends at platform 6831 — listing those as
    # upcoming departures would tell a rider standing at City Hall that a train
    # to City Hall arrives in 3 minutes, which they cannot usefully board.
    heads = []
    head_idx = {}
    candidates = []          # (stop, service, secs, route, headsign, trip_id, seq)
    max_seq = {}
    last_stop = {}
    ...
    with open(path("stop_times.txt"), newline="", encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            ...
            try:
                ...
                seq = -1
            if seq > max_seq.get(trip_id, -1):
                max_seq[trip_id] = seq
                last_stop[trip_id] = r["stop_id"]
            ...
            candidates.append((stop, service, secs, route, headsign, trip_id, seq))
    ...
    relabelled = 0
    for stop, service, secs, route, headsign, trip_id, seq in candidates:
        if seq >= 0 and seq == max_seq.get(trip_id, -1):
            terminating += 1
            continue

        # Calgary signs a handful of late-night trips "City Hall" even though
        # they run through to their real terminus. Trust the final stop over
        # the headsign whenever the headsign names this station.
        if "city hall" in headsign.lower() or "bow valley" in headsign.lower():
            final_stop = last_stop.get(trip_id)
            if final_stop and final_stop not in (STOP_WEST, STOP_EAST):
                repaired = station_label(stop_names.get(final_stop, ""))
                if repaired:
                    headsign = tidy(repaired)
                    relabelled += 1
        if headsign not in head_idx:
            head_idx[headsign] = len(heads)
            heads.append(headsign)
        departures[stop][service].append([secs, route, head_idx[headsign], trip_id])
    ...
    print("  rows carry trip_id for realtime de-duplication")
    print("  %d terminating arrivals excluded (trips ending at City Hall)" % terminating)
    print("  %d mislabelled headsigns repaired from final stop" % relabelled)
